...
sub run {
    ...
    die "Incorrect cluster state\nExpected primary: $expected_primary_db\nCurrent primary: $node_roles->{primary_node}" if
      $expected_primary_db ne $node_roles->{primary_db};
    die "Incorrect cluster state\nExpected failover: $expected_failover_db\nCurrent failover: $node_roles->{failover_node}" if
      $expected_failover_db ne $node_roles->{failover_db};
    ...
    # Wait for takeover
    record_info('Takeover', "Waiting for node '$node_roles->{failover_db}' to become primary");
    wait_for_failed_resources();
    wait_for_takeover(target_node => $node_roles->{failover_db});
    ...
    if ($automatic_register eq 'true') {
        ...
        register_replica(target_hostname => $node_roles->{primary_db}, instance_id => $db_id, switch_user => lc($db_sid) . 'adm');
    }
    ...
    record_info('DB wait', "Waiting for database node '$node_roles->{primary_db}' to start");
    sapcontrol_process_check(
        instance_id => $db_id, expected_state => 'started', wait_for_state => 'yes', timeout => 600);
    record_info('DB started', "All database node '$node_roles->{primary_db}' processes are 'GREEN'");
