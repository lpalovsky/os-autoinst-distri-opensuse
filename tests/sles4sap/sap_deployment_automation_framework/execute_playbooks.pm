...
Executes setup of HanaSR scenario using SDAF ansible playbooks according to:
          https://learn.microsoft.com/en-us/azure/sap/automation/tutorial#sap-application-installation
Playbooks can be found in SDAF repo: https://github.com/Azure/sap-automation/tree/main/deploy/ansible
...
sub playbook_settings {
    ...
    my @supported_components = qw(db_install db_ha nw_pas nw_aas nw_ensa);
    my @component_list = split(',', get_var('SDAF_DEPLOYMENT_SCENARIO'));

    # DB installation pulls in SAP specific configuration
    if (grep /db_install/, @component_list) {
    ...
    if (grep /nw_ensa/, @component_list) {
...
sub run {
    ...
    my @execute_playbooks = playbook_settings();
