...
List hana database processes using C<HDB info> command. Result is presented via C<record_info> call. Returns command
output. Function expects to be executed as sidadm, however you can use B<switch_user> to execute command
using C<sudo su -> as a different user.
...
sub hdb_info {
    my (%args) = @_;
    my $sudo_su = $args{switch_user} ? "sudo su - $args{switch_user} -c" : '';
    my $cmd = join(' ', $sudo_su, '"HDB info"');
    my $out = script_output($cmd, quiet => $args{quiet});
    record_info('HDB info', "Executing '$cmd'\nOutput:\n$out");
    return $out;
}
