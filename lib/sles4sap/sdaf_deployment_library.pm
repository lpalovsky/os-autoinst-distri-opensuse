...
use sles4sap::azure_cli qw(az_vm_name_get);
...
sub sdaf_get_deployer_ip {
    ...
    my $vm_name = sdaf_get_deployer_name();
...
sub replace_tfvars_variables {
    ...
    # Temporary solution for NAT gateway
    set_var('SDAF_DEPLOYMENT_ID', get_var('SDAF_DEPLOYMENT_ID', get_current_job_id()));
    my @variables = qw(
        SDAF_ENV_CODE
        PUBLIC_CLOUD_REGION
        SDAF_DEPLOYMENT_ID
...
=head2 sdaf_destroy_deployer

    sdaf_destroy_deployer();

Destroys deployer VM related to the test run.

=cut

sub sdaf_destroy_deployer {
    my (%args) = @_;
