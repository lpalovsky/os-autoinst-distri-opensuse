...
sub calculate_subnets {
    ...
    # my @sorted_keys = sort { $subnet_definition{$a} =~ m{/(\d+)} <=> $subnet_definition{$b} =~ m{/(\d+)} } keys(%subnet_definition);
    # print(Dumper(@sorted_keys));
    # This will sort subnets according to network size in descending order
    for my $subnet_name (sort {
        # Perl Hashes are unordered, which means, sorting needs to be done here
        my ($b_num) = $subnet_definition{$b} =~ m{/(\d+)};
        my ($a_num) = $subnet_definition{$a} =~ m{/(\d+)};
        $a_num <=> $b_num} keys(%subnet_definition)) {
        print "starting_octet: $starting_octet\n";
